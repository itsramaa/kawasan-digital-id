...
export function FeedbackSurvey({ projectId }: { projectId: string }) {
    ...
    return (
      <div className="bg-card rounded-lg border border-border p-5 space-y-3">
        <h3 className="text-sm font-semibold flex items-center gap-2">
          <CheckCircle className="w-4 h-4 text-primary" /> Feedback Submitted
        </h3>
        <div className="flex items-center gap-1">
          ...
          <span className="text-sm text-muted-foreground ml-2">
            Overall: {existing.overall_rating}/5
          </span>
        </div>
        {existing.comments && <p className="text-sm text-muted-foreground">{existing.comments}</p>}
        <p className="text-xs text-muted-foreground">Submitted {new Date(existing.created_at).toLocaleDateString("id-ID")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-card rounded-lg border border-border p-5 space-y-4">
      <h3 className="text-sm font-semibold flex items-center gap-2">
        <MessageSquare className="w-4 h-4 text-primary" /> How was your experience?
      </h3>
      ...
      <div>
        <label className="text-sm text-muted-foreground">Would you recommend us?</label>
