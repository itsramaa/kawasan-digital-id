...
interface InfraAddOnsProps {
  addOns: TemplateFeature[];
  selectedIds: Set<string>;
  onToggle: (id: string) => void;
}

export function InfraAddOns({ addOns, selectedIds, onToggle }: InfraAddOnsProps) {
  ...
  return (
    <section
      ref={ref}
      className={cn(
        "space-y-4 p-6 rounded-xl bg-muted/50 border border-border transition-all duration-700",
        isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"
      )}
    >
      <h2 className="flex items-center gap-2 text-xl font-bold text-foreground">
        <Server className="w-5 h-5 text-primary" /> Infrastructure Add-Ons
      </h2>
        ...
        <TooltipProvider delayDuration={300}>
          {addOns.map((f) => (
            <label
              key={f.id}
              className={cn(
                "relative flex items-start gap-3 p-3 rounded-lg border bg-card cursor-pointer transition-all duration-200",
                selectedIds.has(f.id)
                  ? "border-primary/50 shadow-[0_0_12px_-3px_hsl(var(--primary)/0.2)]"
