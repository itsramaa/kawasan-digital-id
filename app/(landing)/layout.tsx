...
function Navbar() {
  ...
  return (
    ...
    >
      ...
      {open && (
        <nav className="md:hidden border-t border-border bg-background/95 backdrop-blur-xl px-4 pb-4 space-y-1">
          {navLinks.map((l) => (
            <Link
              key={l.to}
              href={l.to}
              ...
              className={cn(
                ...
                pathname === l.to ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-muted'
              )}
            >
              {l.label}
            </Link>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {navLinks.map((l) => (
                <Link key={l.to} href={l.to} className="block text-xs text-muted-foreground hover:text-foreground transition-colors">
                  {l.label}
                </Link>
              ))}
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              <Link href="/" className="block text-xs text-muted-foreground hover:text-foreground transition-colors">Storefront</Link>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2 text-xs text-muted-foreground">
              <div className="flex items-center gap-2"><Mail className="h-3.5 w-3.5 text-primary flex-shrink-0" />info@kawasandigital.com</div>
              <div className="flex items-center gap-2"><Phone className="h-3.5 w-3.5 text-secondary flex-shrink-0" />+62 812 3456 7890</div>
              <div className="flex items-center gap-2"><MapPin className="h-3.5 w-3.5 text-accent flex-shrink-0" />Jakarta, Indonesia</div>
            </div>
          </div>
        </div>
